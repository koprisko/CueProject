...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 23 14:36:55 2019
...
class NeuralNetwork():
    ...
    def __init__(self):
        ...
        # and mean 0.
        self.synaptic_weights = array([[.5],[.5],[.5],[.1]]) 
        #2 * random.random((4, 1)) - 1
...
class Sentiment:
    def __init__(self, stock, ticker):
        ...
        self.full_articles = [] #used to return articles
...
def stock_lookup(name):
    ...
    company_info = []
    count = 1
    #stud = re.sub(company, "", data)
    for i in range(len(companys)):  
        
        print ("----------------------------")
        print(str(count))
        print(companys[i][8:-2])
        print(tickers[i][10:-1])
        company_info.append((tuple(((companys[i][8:-2]),tickers[i][10:-1]))))
        print("-----------------")  
        count = count + 1   
    index = input("What stock index do you want?")
    if int(index) :
        index = int(index)
        if len(company_info) >= index-1 and index >= 1:
            return company_info[index-1]
...
class Engine:
    def __init__(self, master):
        ...
        #self.master = Canvas(mass, width=700, height=700, border = 0)
        #self.master.pack()
        
        """
        frame = Frame(master, width=200, height=200, background='white')
        frame.pack_propagate(0)    
        frame.pack()
        img = PhotoImage(file="logo.png")
        pic_label = Label(frame, image = img)
        pic_label.pack()
        
        
        pic = ImageTk.PhotoImage(file = "logo.png")
            
        panel = Label(master, image = pic)
        ...
        self.master1 = Canvas(self.master, width=700, height=700, border = 0)
        self.master1.pack()
        
        self.master2 = Canvas(self.master, width=700, height=700, border = 0)
        self.master2.pack()
        
        frame = Frame(self.master, height=10, bg="", colormap="new")
        frame.pack()
        
        self.master3 = Canvas(self.master, width=700, height=700, border = 0)
        self.master3.pack()
        
        frame2 = Frame(self.master, height=10, bg="", colormap="new")
        frame2.pack()
        
        self.master4 = Canvas(self.master, width=700, height=700, border = 0)
        self.master4.pack()
    ...
    def run(self, stock):
            ...
            for i in range(len(sentis)):
                ...
                if i == 0: 
                    ...
                    self.labelI = Label(self.master4, text=string,   border = 0, font=("Times New Roman", 18))
                    self.labelI.pack()
            ...
            csv_creator(ticker, sentiments, '12-10-2015')
            ...
            neural_network.train(array(training_input), array(training_output).T, 10)
            ...
            self.verification['text'] = "We have been predicting movements in this stock with a " + str(val*100) + "% correctness"
            ...
            #canvas = FigureCanvasTkAgg(amount, self)
            #canvas.show()
            #canvas.get_tk_widget().pack(side=self.master.BOTTOM, fill=self.master.BOTH, expand=True)
