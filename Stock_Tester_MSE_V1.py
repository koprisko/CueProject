...
class Financials:
    ...
    def write_bs(self):
        ...
        with open(self.ticker + "_bs.json",'r') as file:
            data = json.load(file)
            for q in data["financials"]:
                quarter = []
                for r in q:
                    quarter.append(q[r])
                    for i in range(len(quarter)-1):
                        quarter[i+1] = float(quarter[i+1])
                all_quarters.append(quarter)
        with open(self.ticker + "_bs.csv",'w') as csvFile:
            writer = csv.writer(csvFile)
            writer.writerows(all_quarters)
    ...
    def write_is(self):
        ...
        with open(self.ticker + "_is.json", 'wb') as output:
            output.write(r.content)
        with open(self.ticker + "_is.json",'r') as file:
            data = json.load(file)
            for q in data["financials"]:
                quarter = []
                for r in q:
                    ...
                    for i in range(len(quarter)-1):
                        quarter[i+1] = float(quarter[i+1])
                all_quarters.append(quarter)
        with open(self.ticker + "_is.csv",'w') as csvFile:
            writer = csv.writer(csvFile)
            writer.writerows(all_quarters)
    ...
    def write_cf(self):
        ...
        with open(self.ticker + "_cf.json", 'wb') as output:
            output.write(r.content)
        with open(self.ticker + "_cf.json",'r') as file:
            data = json.load(file)
            for q in data["financials"]:
                quarter = []
                for r in q:
                    ...
                    for i in range(len(quarter)-1):
                        quarter[i+1] = float(quarter[i+1])
                all_quarters.append(quarter)
        with open(self.ticker + "_cf.csv",'w') as csvFile:
            writer = csv.writer(csvFile)
            writer.writerows(all_quarters)
...
class Valuation:
    ...
    def get_book_value(ticker):
        ...
        book_value = []
        daily_bv = []
        with open(ticker+'_bs.csv') as csvfile:
            readCSV = csv.reader(csvfile, delimiter = ',')
            for row in readCSV:
                share_equity.append(float(row[25]))
        print (share_equity)
        with open(ticker + '_is.csv') as csvfile:
            readCSV = csv.reader(csvfile, delimiter = ',')
            for row in readCSV:
                total_shares.append(float(row[20]))
        print (total_shares)
        for i in range(len(total_shares)):
            book_value.append(share_equity[i]/total_shares[i])
            for k in range(90):
                daily_bv.append(share_equity[i]/total_shares[i])
        return (book_value)
...
def main():
    ...
    data = Fetcher(ticker, [2000,1,1], [2016,1,20],'1d').getHistorical()
